...
class SortAnimationPage extends StatefulWidget {
  const SortAnimationPage({Key? key}) : super(key: key);

  @override
  State<SortAnimationPage> createState() => _SortAnimationPageState();
}

class _SortAnimationPageState extends State<SortAnimationPage> {
  //存放随机数组
  List<int> numbers = [];
  ...
  Future<void> heapify(List<int> arr, int n, int i) async {
    int largest = i;
    int l = 2 * i + 1; // 左子节点索引
    int r = 2 * i + 2; // 右子节点索引
  ...
  ///奇偶排序(Odd-Even Sort) 慢
  Future<void> oddEvenSort() async {
    bool isSorted = false;
  ...
  ///快速排序 快
  Future<void> quickSort(int leftIndex, int rightIndex) async {
  ...
  ///归并排序 快
  Future<void> mergeSort(int leftIndex, int rightIndex) async {
